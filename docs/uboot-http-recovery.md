...
**拦截。** 页面与 C 侧 `httpd_validate()` 各拦一遍：页面不是唯一的客户端，而且页面只认自己那份 `/info`，拿不到或过期了就形同虚设。C 侧在回 200 之前查。查的顺序：勾了「重建 UBI」却没带 U-Boot 文件，400 `rebuilding UBI without a U-Boot FIP would leave nothing to boot`；不重建、也不是整片刷回原厂时，按 `/info` 的方式先挂一次 UBI。挂不上，凡是要写卷的（固件、U-Boot、出厂卷、任意卷）都拒。0.3.0 起少了两条：「只写 BL2」和「没有 `fip` 卷又不带 U-Boot」原来也拒，理由都是「写完一重启就起不来」—— 现在写完不重启了，拒下来只是拦住用户把两份文件里的第一份放进去，所以改成页面上的一句提醒。页面的确认框仍把这几条做成硬错误（不出「仍要写入」）：引导升级没有 UBI 时必须勾重建，有 UBI 没 `fip` 卷时必须带 U-Boot 文件。
...
件事就返回 `FLASH_MORE`，`do_httpd()` 收到就 `continue` 回 `net_loop()`。让
`net_loop()` 肯回来的是 LED 那个超时回调：它本来每 120 ms 醒一次走一格流水，
写入期间顺手 `net_set_state(NETLOOP_SUCCESS)`。**这条时钟不依赖浏览器** ——
关掉页面、拔掉网线，写照样一步步走完。

画灯与「该回去了」因此拆成两个函数：`httpd_led_start()` 只 `led_paint()`，
`httpd_led_tick()` 才设状态。合在一起的话，每次重入 `net_loop()` 的第一拍就
会立刻退出，一个包都来不及服务，整个写入变成空转。

每件事分两拍：先报 `s <文字> [字节]` 再返回，下一拍才真动手。中间隔着一次
`net_loop()`，那一行才来得及送出去 —— 否则页面会顶着上一步的文字干等十秒。

**重入 `net_loop()` 的代价**是 `eth_halt()` + `eth_init()`（`net_loop()` 开头
无条件做）。对 `airoha-gdm1` 这是重置 RX 描述符 + 开关 DMA，`port->id > 1` 那
段带 `phy_startup()` 的代码不走，所以不会重新协商链路；代价是在飞的包会掉，
而 `/wr` 是轮询的，掉了下次再来。`httpd_start_server()` 里加了一道
`if (flash_running)` 提前返回：它平时会把 `up_parts`、`up_base` 全清掉，正在写
的东西必须活过每一次重入。
...
0.3.0 之前是两种图形：流水＝网线还在用，齐闪＝上传结束、设备自己在写、网线随便拔。齐闪的含义整个建立在「连接已经关了」之上，而 A1 之后连接不关 —— 从第一个字节到那个重启框，页面一直连着。于是齐闪没有可说的话，删掉了，`httpd_blink()` 变成 `httpd_chase()`：写入期间 `net_loop()` 的超时回调不跑，改由 cyclic 驱动同一条流水，图形不变。
